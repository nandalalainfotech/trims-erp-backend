...
export class PartController {
	...
	async downloadPdf(@Param('unitslno') unitslno: number,@Req() request: Request, @Res() response: Response) {
		return await this.partService.downloadPdf(unitslno,request, response);
	}
	...
	async downloadExcel(@Param('unitslno') unitslno: number,@Req() request: Request, @Res() response: Response) {
		return await this.partService.downloadExcel(unitslno,request, response);
	}
	...
	findAll(@Param('unitslno') unitslno: number): Promise<Part001mb[]> {
		return this.partService.findAll(unitslno);
	}

...
export class StatutoryPlanController {
	...
	async downloadPdf(@Param('unitslno') unitslno: number,@Req() request: Request, @Res() response: Response) {
		return await this.statutoryPlanService.downloadPdf(unitslno,request, response);
	}
	...
	async downloadExcel(@Param('unitslno') unitslno: number,@Req() request: Request, @Res() response: Response) {
		return await this.statutoryPlanService.downloadExcel(unitslno,request, response);
	}
	...
	findAll(@Param('unitslno') unitslno: number): Promise<Statutory001wb[]> {
		return this.statutoryPlanService.findAll(unitslno);
	}

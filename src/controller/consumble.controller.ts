...
export class ConsumbleController {
	...
	findAll(@Param('unitslno') unitslno: number): Promise<Consumble001mb[]> {
		return this.consumbleService.findAll(unitslno);
	}
	...
	async downloadPdf(@Param('unitslno') unitslno: number,@Req() request: Request, @Res() response: Response) {
		return await this.consumbleService.downloadPdf(unitslno,request, response);
	}
	...
	async downloadExcel(@Param('unitslno') unitslno: number,@Req() request: Request, @Res() response: Response) {
		return await this.consumbleService.downloadExcel(unitslno,request, response);
	}

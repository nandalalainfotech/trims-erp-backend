...
export class FixtureStatusService {
    constructor(
        @InjectRepository(Fixturestatus001mb) private readonly fixturestatusRepository: Repository<Fixturestatus001mb>) {

    }
    async create(fixturestatusDTO: FixtureStatusDTO): Promise<Fixturestatus001mb> {
        const fixturestatus001mb = new Fixturestatus001mb();
        fixturestatus001mb.setProperties(fixturestatusDTO);
        return this.fixturestatusRepository.save(fixturestatus001mb);
    }
    async update(fixturestatusDTO: FixtureStatusDTO): Promise<Fixturestatus001mb> {
        const fixturestatus001mb = new Fixturestatus001mb();
        fixturestatus001mb.setProperties(fixturestatusDTO);
        await this.fixturestatusRepository.update({ slNo: fixturestatus001mb.slNo }, fixturestatus001mb);
        return fixturestatus001mb;
    }

    async findAll(unitslno:any): Promise<Fixturestatus001mb[]> {
        return await this.fixturestatusRepository.find({order: { slNo: "DESC" },where:{unitslno:unitslno}});
    }

    findOne(id: number): Promise<Fixturestatus001mb> {
        return this.fixturestatusRepository.findOne(id);
    }
    async remove(id: number): Promise<void> {
		await this.fixturestatusRepository.delete(id);
	}


    async downloadPdf(unitslno:any,@Req() request: Request, @Res() response: Response) {
        let status = await this.fixturestatusRepository.find({where:{unitslno:unitslno}});
        var fs = require('fs');
        var pdf = require('dynamic-html-pdf');
        var html = fs.readFileSync('fixturestatus.html', 'utf8');
        ...
        var document = {
            ...
            path: "./pdf/fixturestatus.pdf"    // it is not required if type is buffer
        };
        ...
        if (document === null) {
            ...
            pdf.create(document, options).then((pathRes) => {
                ...
                response.writeHead(200, {
                    "Content-Disposition": "attachment;filename=" + "fixturestatus.pdf",
    ...
    async downloadExcel(unitslno:any,@Req() request: Request, @Res() response: Response) {
        ...
        if (status.length < 0) {
            ...
            let worksheet = workbook.addWorksheet('fixture_status_reports'); // creating worksheet
            ...
            worksheet.getCell('B3:D4').value = "LIST OF FIXTURE STATUS DETAILS";
            ...
            worksheet.getCell('B5').value = "Fixture Code Group";

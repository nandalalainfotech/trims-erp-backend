...
@Injectable()
export class FixturePreventiveCheckListService {
    constructor(
        @InjectRepository(Fixturepreventivechecklist001wb) private readonly fixpreCheckRepository: Repository<Fixturepreventivechecklist001wb>) {
    }

    async create(fixturepreventiveChecklistDTOs: FixturePreventiveChecklistDTO[]): Promise<Fixturepreventivechecklist001wb[]> {
        for (let i = 0; i < fixturepreventiveChecklistDTOs.length; i++) {
            let fixturepreventivechecklist001wb = new Fixturepreventivechecklist001wb();
            fixturepreventivechecklist001wb.setProperties(fixturepreventiveChecklistDTOs[i]);
            await this.fixpreCheckRepository.save(fixturepreventivechecklist001wb);
        }
        let checklist = await this.fixpreCheckRepository.find();
        return checklist;

    }

    async update(fixturepreventiveChecklistDTOs: FixturePreventiveChecklistDTO): Promise<Fixturepreventivechecklist001wb> {
        let fixturepreventivechecklist001wb = new Fixturepreventivechecklist001wb();
        fixturepreventivechecklist001wb.setProperties(fixturepreventiveChecklistDTOs);
        await this.fixpreCheckRepository.update({ slNo: fixturepreventivechecklist001wb.slNo }, fixturepreventivechecklist001wb);
        return fixturepreventivechecklist001wb;
    }

    async findAll(unitslno:any): Promise<Fixturepreventivechecklist001wb[]> {
        return this.fixpreCheckRepository.find({ order: { slNo: "DESC" },relations: ["mslno2", "cpslno2"] , where: { unitslno:unitslno }});
    }

    async findAllByFixtureId(mslno: number,unitslno:any): Promise<Fixturepreventivechecklist001wb[]> {
        return this.fixpreCheckRepository.find({ order: { slNo: "DESC" },relations: ["mslno2", "cpslno2"], where: { "mslno": mslno, unitslno:unitslno } });
    }

    findOne(id: number): Promise<Fixturepreventivechecklist001wb> {
        return this.fixpreCheckRepository.findOne(id);
    }

    async remove(id: string): Promise<void> {
        await this.fixpreCheckRepository.delete(id);
    }

    async downloadPdf(mslno: number,unitslno:any, @Req() request: Request, @Res() response: Response) {
        let preventivechecks = await this.fixpreCheckRepository.find({
            ...
            where: { mslno:mslno,unitslno:unitslno },
             ...
             if (preventivechecks.length < 0) {
                   ...
                   for (let i = 0; i < preventivechecks.length; i++) {
                       ...
                       worksheet.getCell('B' + temp).value = preventivechecks[i].mslno2.fcode;
       
                       worksheet.mergeCells('C' + temp);
                       worksheet.getCell('C' + temp).value = preventivechecks[i].mslno2.fname;
                       worksheet.mergeCells('D' + temp);
                    //    worksheet.getCell('D' + temp).value = preventivechecks[i].cpslno2.fcheckpoints;

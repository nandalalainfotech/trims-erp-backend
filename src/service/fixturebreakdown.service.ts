...
@Injectable()
export class FixtureBreakdownService {

    constructor(
        @InjectRepository(Fixturebreakdown001mb) private readonly fixturebreakdownRepository: Repository<Fixturebreakdown001mb>) {

    }
    async create(fixturebreakdownDTO: FixtureBreakdownDTO): Promise<Fixturebreakdown001mb> {
        const fixturebreakdown001mb = new Fixturebreakdown001mb();
        fixturebreakdown001mb.setProperties(fixturebreakdownDTO);
        return this.fixturebreakdownRepository.save(fixturebreakdown001mb);
    }
    async update(fixturebreakdownDTO: FixtureBreakdownDTO): Promise<Fixturebreakdown001mb> {
        const fixturebreakdown001mb = new Fixturebreakdown001mb();
        fixturebreakdown001mb.setProperties(fixturebreakdownDTO);
        await this.fixturebreakdownRepository.update({ slNo: fixturebreakdown001mb.slNo }, fixturebreakdown001mb);
        return fixturebreakdown001mb;
    }

    async findAll(unitslno:any): Promise<Fixturebreakdown001mb[]> {
        return await this.fixturebreakdownRepository.find({order: { slNo: "DESC" }, relations: ["sslno2"],where:{unitslno:unitslno}});
    }

    findOne(id: number): Promise<Fixturebreakdown001mb> {
        return this.fixturebreakdownRepository.findOne(id);
    }
    async remove(slNo: number): Promise<void> {
        await this.fixturebreakdownRepository.delete(slNo);
    }



    async downloadPdf(unitslno:any,@Req() request: Request, @Res() response: Response) {
        let breaks = await this.fixturebreakdownRepository.find({ relations: ["sslno2"],where:{unitslno:unitslno} });
        var fs = require('fs');
        var pdf = require('dynamic-html-pdf');
        var html = fs.readFileSync('fixturebreakdown.html', 'utf8');
        ...
        var document = {
            ...
            path: "./pdf/fixturebreakdown.pdf"    // it is not required if type is buffer
        };
        ...
        if (document === null) {
            ...
            pdf.create(document, options).then((pathRes) => {
                ...
                response.writeHead(200, {
                    "Content-Disposition": "attachment;filename=" + "fixturebreakdown.pdf",
    ...
    async downloadExcel(unitslno:any,@Req() request: Request, @Res() response: Response) {
        ...
        if (breaks.length < 0) {
            ...
            worksheet.getCell('B3:C4').value = "LIST OF FIXTURE BREAKDOWN DETAILS";
            ...
            for (let i = 0; i < breaks.length; i++) {
                ...
                worksheet.getCell('B' + temp).value = breaks[i].fbname;

                worksheet.mergeCells('C' + temp);
                worksheet.getCell('C' + temp).value = breaks[i].fbdetails;

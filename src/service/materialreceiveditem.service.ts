...
export class MaterialreceiveditemService {
    ...
    async findAll(unitslno: any): Promise<Materialreceiveditem001wb[]> {
        return await this.materialreceiveditemRepository.find({order: { slNo: "DESC" }, relations: ["itemcode2", "cucode2", "cptcode2", "prtcode2"], where: { unitslno: unitslno } })
    }
    async create(materialreceiveditemDTO: MaterialreceiveditemDTO): Promise<Materialreceiveditem001wb> {
        ...
        materialreceiveditem001wb.setProperties(materialreceiveditemDTO);
        return this.materialreceiveditemRepository.save(materialreceiveditem001wb);
    }
    async update(materialreceiveditemDTO: MaterialreceiveditemDTO): Promise<Materialreceiveditem001wb> {
        const materialreceiveditem001wb = new Materialreceiveditem001wb();
        materialreceiveditem001wb.setProperties(materialreceiveditemDTO);
        await this.materialreceiveditemRepository.update({ slNo: materialreceiveditem001wb.slNo }, materialreceiveditem001wb);
        return materialreceiveditem001wb;
    }
    ...
    async downloaditemPdf(unitslno: any, @Req() request: Request, @Res() response: Response) {
        ...
        let item = await this.materialreceiveditemRepository.find({

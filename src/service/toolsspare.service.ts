...
@Injectable()
export class ToolsSpareService {
    constructor(
        @InjectRepository(Toolsspares001mb) private readonly toolsspareRepository: Repository<Toolsspares001mb>) {

    }
    async create(toolssparesDTO: ToolsSparesDTO): Promise<Toolsspares001mb> {
        const toolsspares001mb = new Toolsspares001mb();
        toolsspares001mb.setProperties(toolssparesDTO);
        return this.toolsspareRepository.save(toolsspares001mb);
    }
    async update(toolssparesDTO: ToolsSparesDTO): Promise<Toolsspares001mb> {
        const toolsspares001mb = new Toolsspares001mb();
        toolsspares001mb.setProperties(toolssparesDTO);
        await this.toolsspareRepository.update({ slNo: toolsspares001mb.slNo }, toolsspares001mb);
        return toolsspares001mb;
    }

    async findAll(unitslno:any): Promise<Toolsspares001mb[]> {
        return await this.toolsspareRepository.find({order: { slNo: "DESC" }, relations: ["msslno2","tsslno2"] ,where:{unitslno:unitslno}});
    }

    findOne(id: number): Promise<Toolsspares001mb> {
        return this.toolsspareRepository.findOne(id);
    }
    async remove(id: number): Promise<void> {
		await this.toolsspareRepository.delete(id);
	}



    async downloadPdf(unitslno:any,@Req() request: Request, @Res() response: Response) {
        let spares = await this.toolsspareRepository.find({where:{unitslno:unitslno}, relations: ["msslno2","tsslno2"] });
        var fs = require('fs');
        var pdf = require('dynamic-html-pdf');
        var html = fs.readFileSync('toolsspare.html', 'utf8');
        ...
        var document = {
            ...
            path: "./pdf/toolsspare.pdf"    // it is not required if type is buffer
        };
        ...
        if (document === null) {
            ...
            pdf.create(document, options).then((pathRes) => {
                ...
                response.writeHead(200, {
                    "Content-Disposition": "attachment;filename=" + "toolsspare.pdf",
    ...
    async downloadExcel(unitslno:any,@Req() request: Request, @Res() response: Response) {
        ...
        if (spares.length < 0) {
            ...
            worksheet.getCell('C3:H4').value = "LIST OF TOOLS & DIE SPARES DETAILS";
            ...
            for (let i = 0; i < spares.length; i++) {
                ...
                worksheet.getCell('B' + temp).value = spares[i].msslno2.tcode;

                worksheet.mergeCells('C' + temp);
                worksheet.getCell('C' + temp).value = spares[i].tspares;
                worksheet.mergeCells('D' + temp);
                worksheet.getCell('D' + temp).value = spares[i].tsparescost;

                worksheet.mergeCells('E' + temp);
                worksheet.getCell('E' + temp).value = spares[i].tspecification;

                worksheet.mergeCells('F' + temp);
                worksheet.getCell('F' + temp).value = spares[i].tleadtime;

                worksheet.mergeCells('G' + temp);
                worksheet.getCell('G' + temp).value = spares[i].tminimumstocklevel;
                
                
                worksheet.mergeCells('H' + temp);
                worksheet.getCell('H' + temp).value = spares[i].treorderlevel;
                
                worksheet.mergeCells('I' + temp);
                worksheet.getCell('I' + temp).value = spares[i].tsslno2.name;

...
export class RootcauseService {
    ...
    async findAll(unitslno:number, ): Promise<Rootcause001mb[]> {
        return await this.RootcauseRepository.find({ order: { slNo: "DESC" },relations: ["sslno2","brslno2"], where: { 'unitslno': unitslno } });
    }
    ...
    async downloadPdf(unitslno:any,@Req() request: Request, @Res() response: Response) {
        let rootcause = await this.RootcauseRepository.find({ relations: ["sslno2","brslno2"],where:{unitslno:unitslno} });
        var fs = require('fs');
        var pdf = require('dynamic-html-pdf');

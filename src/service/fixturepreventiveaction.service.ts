...
@Injectable()
export class FixturePreventiveactionService {

    constructor(
        @InjectRepository( Fixturepreventiveaction001mb) private readonly fixturePreventiveactionRepository: Repository< Fixturepreventiveaction001mb>) {

    }
    async create(fixturepreventiveactionDTO: FixturePreventiveactionDTO): Promise< Fixturepreventiveaction001mb> {
        const fixturepreventiveaction001mb = new  Fixturepreventiveaction001mb();
        fixturepreventiveaction001mb.setProperties(fixturepreventiveactionDTO);
        return this.fixturePreventiveactionRepository.save(fixturepreventiveaction001mb);
    }
    async update(fixturepreventiveactionDTO: FixturePreventiveactionDTO): Promise< Fixturepreventiveaction001mb> {
        const fixturepreventiveaction001mb = new  Fixturepreventiveaction001mb();
        fixturepreventiveaction001mb.setProperties(fixturepreventiveactionDTO);
        await this.fixturePreventiveactionRepository.update({ slNo: fixturepreventiveaction001mb.slNo }, fixturepreventiveaction001mb);
        return fixturepreventiveaction001mb;
    }

    async findAll(unitslno:any): Promise< Fixturepreventiveaction001mb[]> {
        return await this.fixturePreventiveactionRepository.find({ order: { slNo: "DESC" },relations: ["sslno2", "rcslno2"], where:{unitslno:unitslno} });
    }

    async findAllbyRootCauseId(rcslno: number): Promise< Fixturepreventiveaction001mb[]> {
        return this.fixturePreventiveactionRepository.find({order: { slNo: "DESC" }, select: ["slNo","name"], where: { "rcslno": rcslno } });
    }

    findOne(id: number): Promise< Fixturepreventiveaction001mb> {
        return this.fixturePreventiveactionRepository.findOne(id);
    }
    async remove(slNo: number): Promise<void> {
        await this.fixturePreventiveactionRepository.delete(slNo);
    }

    async downloadPdf(unitslno:any,@Req() request: Request, @Res() response: Response) {
        let preventive = await this.fixturePreventiveactionRepository.find({ where:{unitslno:unitslno},relations: ["sslno2","rcslno2"] });
        var fs = require('fs');
        var pdf = require('dynamic-html-pdf');
    ...
    async downloadExcel(unitslno:any,@Req() request: Request, @Res() response: Response) {
        ...
        if (preventive.length < 0) {
            ...
            for (let i = 0; i < preventive.length; i++) {
                ...
                worksheet.getCell('B' + temp).value = preventive[i].rcslno2.fname 

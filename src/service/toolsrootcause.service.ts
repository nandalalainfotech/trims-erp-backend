...
@Injectable()
export class ToolsRootcauseService {

    constructor(
        @InjectRepository(Toolsrootcause001mb) private readonly toolsRootcauseRepository: Repository<Toolsrootcause001mb>) {

    }
    async create(toolsRootcauseDTO: ToolsRootcauseDTO): Promise<Toolsrootcause001mb> {
        const toolsrootcause001mb = new Toolsrootcause001mb();
        toolsrootcause001mb.setProperties(toolsRootcauseDTO);
        return this.toolsRootcauseRepository.save(toolsrootcause001mb);
    }
    async update(toolsRootcauseDTO: ToolsRootcauseDTO): Promise<Toolsrootcause001mb> {
        const toolsrootcause001mb = new Toolsrootcause001mb();
        toolsrootcause001mb.setProperties(toolsRootcauseDTO);
        await this.toolsRootcauseRepository.update({ slNo: toolsrootcause001mb.slNo }, toolsrootcause001mb);
        return toolsrootcause001mb;
    }

    async findAll(unitslno:any): Promise<Toolsrootcause001mb[]> {
        return await this.toolsRootcauseRepository.find({ order: { slNo: "DESC" },relations: ["sslno2","tbrslno2"],where:{unitslno:unitslno} });
    }

    async findAllbyBreakDownId(bdslno: number): Promise<Toolsrootcause001mb[]> {
        return this.toolsRootcauseRepository.find({ order: { slNo: "DESC" },select: ["slNo","tname"], where: { "tbrslno": bdslno } });
    }

    findOne(id: number): Promise<Toolsrootcause001mb> {
        return this.toolsRootcauseRepository.findOne(id);
    }
    async remove(slNo: number): Promise<void> {
        await this.toolsRootcauseRepository.delete(slNo);
    }

    async downloadPdf(unitslno:any,@Req() request: Request, @Res() response: Response) {
        let rootcause = await this.toolsRootcauseRepository.find({where:{unitslno:unitslno}, relations: ["sslno2","tbrslno2"] });
        var fs = require('fs');
        var pdf = require('dynamic-html-pdf');
        var html = fs.readFileSync('toolsrootcause.html', 'utf8');
        ...
        var document = {
            ...
            path: "./pdf/toolsrootcause.pdf"    // it is not required if type is buffer
        };
        ...
        if (document === null) {
            ...
            pdf.create(document, options).then((pathRes) => {
                ...
                response.writeHead(200, {
                    "Content-Disposition": "attachment;filename=" + "toolsrootcause.pdf",
    ...
    async downloadExcel(unitslno:any,@Req() request: Request, @Res() response: Response) {
      ...
      if (rootcause.length < 0) {
            ...
            for (let i = 0; i < rootcause.length; i++) {
                ...
                worksheet.getCell('B' + temp).value = rootcause[i].tbrslno2.tbname 

                worksheet.mergeCells('C' + temp);
                worksheet.getCell('C' + temp).value = rootcause[i].tname;
                worksheet.mergeCells('D' + temp);
                worksheet.getCell('D' + temp).value = rootcause[i].tdetails;
